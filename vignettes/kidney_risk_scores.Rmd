...
The Scaling Factor is the median KDRI for kidney donors in the USA in the previous year, and is published each year on the OPTN website. For 2019 the scaling factor is approximately 1.250609. This normalises the donor risk within each year, but I think it obscures inter-year comparisons. The `scaling` parameter defaults to 1 so can be left out when a non-scaled KDRI is desired.
...
my_data5 = my_data2 %>%
  mutate(USKDRI = uskdri(age = Donor.Age, height = Donor.Height, weight = Donor.Weight,
                         ...
                         hcv = Donor.HepatitisC, dcd = Donor.Type, scaling = 1.250609, units = "US"))
```
